...
export default function Profile() {
  ...
  useEffect(() => {
    if (!user) return;
    return subscribeEvents(setAllEvents);
  }, [user]);

  useEffect(() => {
    if (!user) return;
    return subscribeUser(user.uid, setCurrentUserData);
  }, [user]);
  ...
  useEffect(() => {
    ...
    Promise.all(friends.map((uid) => getAppUser(uid)))
      .then((profiles) => setFriendProfiles(profiles.filter(Boolean) as AppUser[]));
  }, [currentUserData?.friends]);

  const myEvents = allEvents.filter((e) => e.creatorId === user?.uid);
  ...
  async function handleSearch() {
    ...
    try {
      ...
      if (results.length === 0)
        toast({ title: "No users found", description: "Try a different name or email." });
    } finally {
      setSearching(false);
    }
  }
    ...
    <div className="min-h-screen bg-background text-foreground pb-20">
      <div className="max-w-2xl mx-auto px-4">
        <div className="flex items-center justify-between pt-6 mb-4">
          ...
          <button data-testid="button-logout" onClick={handleLogout} className="text-sm text-destructive hover:opacity-80">
            Sign Out
          </button>
        </div>
          ...
          <div className="space-y-3">
            {myEvents.length === 0 ? (
              <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground text-sm">
                No events yet.{" "}
                <button onClick={() => setLocation("/events/new")} className="text-primary hover:underline">Create one</button>
              </div>
            ) : (
        ...
        {tab === "going" && (
          <div className="space-y-3">
            {goingEvents.length === 0 ? (
              <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground text-sm">
                You haven't RSVPed "Going" to any events yet.{" "}
                <button onClick={() => setLocation("/")} className="text-primary hover:underline">Explore events</button>
              </div>
            ) : (
                    ...
                    return (
                      <div key={result.uid} className="bg-card border border-border rounded-xl p-3 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-foreground cursor-pointer hover:text-primary" onClick={() => setLocation(`/users/${result.uid}`)}>
                            {result.displayName}
                          </p>
